# sports_engine.py
"""BetAnalyzer Pro - Moteur d'analyse complet à 16 critères (Format Compact & Fiable)."""
...
def calculate_form(last_5):
    ...
    return {"results": last_5, "points": pts, "max_points": len(last_5)*3, "score": score, "label": "Bonne" if score>=50 else "Faible"}
...
def calculate_stakes(situation):
    b = {"title_race": 20, "relegation": 25, "european_qualification": 15, "african_qualification": 15, "mid_table": 0}.get(situation, 0)
    return {"situation": situation, "label": situation, "bonus": b, "score": 50 + b}
...
def calculate_momentum(streak):
    ...
    return {"type": st, "count": count, "bonus": b, "label": "Série", "score": max(0, min(100, 50+b))}
...
def calculate_chemistry(stability=None, xi_reg=None):
    ...
    return {"squad_stability": round(s*100), "xi_regularity": round(x*100), "score": score, "label": "Bonne"}

def calculate_missing_impact(missing=None):
    if not missing: missing = []
    malus = min(30, sum(8 if p.get("importance",50)>=85 else 5 for p in missing))
...
def calculate_fatigue(days_rest):
    ...
    return {"days_rest": days_rest, "malus_percent": 8 if days_rest<4 else 0, "score": score, "label": "Repos"}

def calculate_clean_sheet(cs, total):
    tot = max(1, total)
    ratio = (cs / tot) * 100
    return {"clean_sheets": cs, "total_matches": tot, "ratio": round(ratio), "score": round(ratio), "label": "Clean Sheet"}

def calculate_discipline(y_avg, r_avg, f_avg):
    score = max(0, min(100, round(100 - (y_avg*5 + r_avg*25 + f_avg*2))))
    return {"yellow_avg": round(y_avg,2), "red_avg": round(r_avg,3), "fouls_avg": round(f_avg,1), "risk_score": score, "risk_label": "Discipliné", "score": score}
...
def apply_live_correction(pre_score, live_data):
    ...
    w_live = min(0.9, m / 90)
    bias = max(5, min(95, 50 + (sh - sa)*10))
    return {**pre_score, "confidence": round(pre_score["confidence"] * (1 - w_live) + bias * w_live), "live_bias": bias, "minute": m, "corrected": True}

def calculate_global_confidence(scores):
    w_sum = sum(scores.get(k,50) * w for k,w in WEIGHTS.items())
    conf = max(0, min(96, round(w_sum)))
    return {"confidence": conf, "label": "🟢 Confiance élevée" if conf>=75 else "🟡 Confiance modérée", "prediction": "Pari sûr" if conf>=75 else "Équilibré"}
...
def simulate_match(match):
    ...
    h2h = calculate_h2h([])
    s_h, s_a = calculate_stakes("title_race"), calculate_stakes("mid_table")
    h_ha, a_ha = calculate_home_away(True, 0.8, 0.5), calculate_home_away(False, 0.8, 0.5)
    ...
    h_m, a_m = calculate_momentum({"type":"W","count":2}), calculate_momentum({"type":"L","count":1})
    ...
    h_fat, a_fat = calculate_fatigue(5), calculate_fatigue(5)
    h_cs, a_cs = calculate_clean_sheet(4, 10), calculate_clean_sheet(3, 10)
    h_disc, a_disc = calculate_discipline(2.0, 0.1, 10.0), calculate_discipline(2.0, 0.1, 10.0)

    h_scores = {"form": h_form["score"], "ranking": h_rank["score"], "h2h": h2h["score"], "stakes": s_h["score"], "home_away": h_ha["score"], "goals": h_g["score"], "momentum": h_m["score"]}
    a_scores = {"form": a_form["score"], "ranking": a_rank["score"], "h2h": 100 - h2h["score"], "stakes": s_a["score"], "home_away": a_ha["score"], "goals": a_g["score"], "momentum": a_m["score"]}
    ...
    if diff > 10: pred, pred_lbl, conf = "1", "Victoire Domicile", h_glob["confidence"]
    elif diff < -10: pred, pred_lbl, conf = "2", "Victoire Extérieur", a_glob["confidence"]
    else: pred, pred_lbl, conf = "N", "Match Nul probable", round((h_glob["confidence"]+a_glob["confidence"])/2)

    safety = "+2.5 Buts" if ou["over_25"]>65 else ("BTTS OUI" if ou["btts_yes"]>65 else "1N Double Chance")
...
def apply_rolling_window(matches, days_past=2, days_future=7):
    now = datetime.utcnow()
    cutoff_p, cutoff_f = now - timedelta(days=days_past), now + timedelta(days=days_future)
    act, exp = [], []
    for m in matches:
        try: m_date = datetime.fromisoformat(m["utcDate"].replace("Z", "+00:00")).replace(tzinfo=None)
        except: m_date = now
        if m_date < cutoff_p or m_date > cutoff_f: exp.append(m)
        else: act.append(m)
...
def filter_team_collision(matches):
    ...
    for m in matches:
        ...
        if (hid and hid in busy_days[d_key]) or (aid and aid in busy_days[d_key]): continue
        if hid: busy_days[d_key].add(hid)
        if aid: busy_days[d_key].add(aid)
        filtered.append(m)
    return filtered

def enrich_match(match):
    if match.get("status") in LIVE_STATUSES:
        match["live_data"] = {"is_live": True, "minute": match.get("minute", random.randint(1,90)), "score_home": match.get("score",{}).get("fullTime",{}).get("home") or 0, "score_away": match.get("score",{}).get("fullTime",{}).get("away") or 0}
...
def get_demo_matches():
    ...
    for c in comps:
        ...
        for i in range(min(2, len(t_list)//2)):
            ...
            d_off = random.randint(0, 4)
            m_date = now + timedelta(days=d_off, hours=random.randint(1, 12))
            st = "IN_PLAY" if (d_off == 0 and i == 0) else "SCHEDULED"
            sh = random.randint(0,2) if st=="IN_PLAY" else None
            sa = random.randint(0,2) if st=="IN_PLAY" else None
            m = {
                ...
                "utcDate": m_date.isoformat()+"Z", "status": st, "matchday": 24,
                "score": {"fullTime": {"home": sh, "away": sa}}, "minute": random.randint(10,85) if st=="IN_PLAY" else None, "category": "Masculin"
            }
